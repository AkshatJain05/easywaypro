...
router.get("/quizSet/:quizId", async (req, res) => {
  try {
...
router.get("/user-score", protect, async (req, res) => {
  try {
    // console.log("Hello")
  ...
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.type === "code" && (!q.answerHint || !q.answerHint.trim())) {
        return res.status(400).json({
          message: `Question ${i + 1} is a code question but answer hint is empty`,
        });
      }
    }
...
router.get("/complete-quiz", protect, async (req, res) => {
  try {
