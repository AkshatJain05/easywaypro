...
import {
  FiCalendar, FiClock, FiPlus, FiTrash2, FiCheck, FiActivity,
  FiBell, FiSun, FiMoon, FiCoffee, FiCheckCircle, FiTarget,
  FiZap, FiBox, FiGrid, FiTrash, FiX, FiAward, FiEdit3,
  FiMap, FiChevronLeft, FiChevronRight, FiSave,
} from "react-icons/fi";
...
const RadialProgress = ({ value = 0, color = INDIGO, size = 90 }) => {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 96 96" className="shrink-0">
      ...
      />
      <text x="48" y="45" textAnchor="middle" fill="white" fontSize="16" fontWeight="700">{value}</text>
      <text x="48" y="58" textAnchor="middle" fill="rgba(255,255,255,0.28)" fontSize="9">%</text>
    </svg>
  );
};
...
const StatCard = ({ icon, label, value, accent }) => (
  <div className="bg-[#111120] border border-white/[0.06] rounded-2xl p-4 flex items-center gap-3 hover:border-white/10 transition-all min-w-0">
    ...
    <div className="min-w-0">
      ...
      <p className="text-[9px] font-bold uppercase tracking-widest text-white/25 mt-0.5 truncate">{label}</p>
    </div>
  </div>
);
        ...
        <div className="bg-[#0f0f1e] border border-white/[0.08] rounded-t-3xl sm:rounded-3xl p-6 w-full max-w-md shadow-2xl">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              {isEdit ? <FiEdit3 className="text-indigo-400" size={14} /> : <FiPlus className="text-indigo-400" size={14} />}
              <span className="text-xs font-black uppercase tracking-widest text-white">
                {isEdit ? "Edit task" : "New task"}
              </span>
            </div>
            <button onClick={onClose} className="p-1.5 hover:bg-white/5 rounded-lg transition"><FiX size={13} /></button>
          </div>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="relative">
                <FiCalendar className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/20 pointer-events-none" size={12} />
              ...
              <div className="relative">
                <FiClock className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/50 pointer-events-none" size={12} />
            ...
            <button type="submit"
              ...
              {isEdit ? <><FiSave size={14} /> Save changes</> : <><FiPlus size={14} /> Deploy task</>}
            </button>
          </form>
        </div>
      </motion.div>
...
// ─── Visual Roadmap ───────────────────────────────────────────────────────────
const VisualRoadmap = ({ tasks, onToggle, onDelete, onEdit, page, onPageChange }) => {
  ...
  return (
    <div>
              ...
              <motion.div key={task._id}
                ...
                <button onClick={() => onToggle(task._id)}
                  ...
                  {task.completed
                    ...
                    : <span className="text-[9px] font-black text-white/20">{gIdx + 1}</span>}
                </button>
                {/* Connector */}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <button onClick={() => onToggle(task._id)}
                      className={`sm:hidden mt-1 w-5 h-5 rounded-full border-2 shrink-0 flex items-center justify-center ${
                        task.completed ? "bg-green-700 border-indigo-400" : "border-white/20 hover:border-indigo-400"
                      }`}>
                      {task.completed && <FiCheck size={8} className="text-white font-bold" />}
                    </button>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span className="text-[8px] font-black text-white/50 uppercase">#{gIdx + 1}</span>
                        {task.completed && (
                          <span className="text-[9px] font-black text-white bg-green-700 px-1.5 py-0.5 rounded-full border border-indigo-500/20">Done</span>
                        )}
                      </div>
                      <p className={`text-sm font-semibold leading-snug break-words ${task.completed ? "line-through text-gray-400" : "text-white"}`}>
                        {task.text}
                      </p>
                      {(task.date || task.time) && (
                        <div className="flex flex-wrap gap-3 mt-2">
                          {task.date && <span className="flex items-center gap-1 text-[10px] text-white/50 font-mono"><FiCalendar size={8} className="text-blue-400" /> {task.date}</span>}
                          {task.time && <span className="flex items-center gap-1 text-[10px] text-white/50 font-mono"><FiClock size={8} className="text-blue-400" /> {task.time}</span>}
                        </div>
                      )}
                    </div>
      ...
      <Pagination page={page} total={tasks.length} pageSize={PAGE_SIZE} onChange={onPageChange} />
    </div>
  );
};
...
const EmptyState = ({ onAdd }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
    ...
    <div className="text-center px-4">
      ...
      <p className="text-white/25 text-sm">Deploy your first objective to get started.</p>
    </div>
...
useEffect(() => {
  ...
  const filteredTasks = useMemo(() => {
    const base = [...tasks].sort((a, b) => Number(a.completed) - Number(b.completed));
  ...
  const timetablePeriods = useMemo(() => [
    { id: "morning",   label: "Morning",   icon: <FiSun size={13} className="text-amber-400" />,   check: h => h >= 5  && h < 12 },
    { id: "afternoon", label: "Afternoon", icon: <FiCoffee size={13} className="text-sky-400" />,  check: h => h >= 12 && h < 17 },
    { id: "evening",   label: "Evening",   icon: <FiMoon size={13} className="text-violet-400" />, check: h => h >= 17 || h < 5  },
  ].map(p => ({
    ...p,
    items: filteredTasks.filter(t => p.check(parseInt(t.time?.split(":")[0] ?? 0))),
  })), [filteredTasks]);
...
const notifyUser = (title, body) => {
  ...
  const addTask = async ({ text, date, time }) => {
    try {
      const res = await axios.post(`${API_URL}/tasks`, { text, date, time }, { withCredentials: true });
  ...
  const saveEdit = async ({ text, date, time }) => {
    try {
      const res = await axios.put(`${API_URL}/tasks/${editTask._id}/edit`, { text, date, time }, { withCredentials: true });
  ...
  const toggleTask = async (id) => {
    try {
      const res = await axios.put(`${API_URL}/tasks/${id}/toggle`, {}, { withCredentials: true });
  ...
  const clearAll = async () => {
    try {
      ...
      setTasks([]); setConfirmClear(false);
      toast.success("Workspace cleared");
    } catch { toast.error("Action forbidden"); }
  };
...
useEffect(() => {
  ...
  useEffect(() => { setRoadmapPage(1); setGridPage(1); }, [filter, activeTab]);
  ...
  return (
    <div className="min-h-screen bg-[#08080f] text-white font-sans selection:bg-indigo-500/25">
      <Toaster position="bottom-right" toastOptions={{
        style: { background: "#13131f", color: "#e2e8f0", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "14px", fontSize: "13px" },
      }} />
      ...
      <AnimatePresence>
        ...
        {showNotif && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40" onClick={() => setShowNotif(false)} />
            <motion.aside initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.05]">
                <span className="text-xs font-black text-white uppercase tracking-widest">Pending tasks</span>
                <button onClick={() => setShowNotif(false)} className="p-1.5 hover:bg-white/5 rounded-lg"><FiX size={13} /></button>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                {tasks.filter(t => !t.completed).length === 0 ? (
                  ...
                  <div key={t._id} className="p-3.5 bg-white/[0.02] border border-white/[0.05] rounded-xl">
                    <p className="text-xs text-white font-semibold leading-snug">{t.text}</p>
                    {(t.date || t.time) && <p className="text-[9px] text-white/20 font-mono mt-1.5">{t.date} {t.time}</p>}
                  </div>
                ))}
              </div>
            </motion.aside>
        ...
        {confirmClear && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" onClick={() => setConfirmClear(false)} />
            <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }}
              ...
              <div className="bg-[#0f0f1e] border border-white/[0.08] rounded-3xl p-7 w-full max-w-sm shadow-2xl">
                ...
                <h3 className="text-white font-black text-lg mb-1.5">Clear all tasks?</h3>
                <p className="text-white/30 text-sm mb-6">This permanently deletes all data.</p>
                <div className="flex gap-3">
                  <button onClick={() => setConfirmClear(false)} className="flex-1 py-2.5 rounded-xl border border-white/10 text-sm font-bold hover:bg-white/5 transition">Cancel</button>
                  <button onClick={clearAll} className="flex-1 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-400 text-white text-sm font-bold transition">Delete all</button>
                </div>
              </div>
            </motion.div>
          </>
        ...
        {showForm && <TaskFormModal onClose={() => setShowForm(false)} onSubmit={addTask} />}
        {editTask  && <TaskFormModal onClose={() => setEditTask(null)} onSubmit={saveEdit} initial={editTask} />}
      </AnimatePresence>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        ...
        <header className="flex items-center justify-between mb-6 gap-3 flex-wrap">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base font-black text-white leading-none truncate">Task <span className="text-indigo-400">Planner</span></h1>
              <p className="text-[8px] text-white/50 font-bold uppercase tracking-widest mt-0.5">Dashboard</p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={() => setShowNotif(true)}
              ...
              <FiBell size={14} />
              {stats.pending > 0 && <span className="absolute top-2 right-2 w-1.5 h-1.5 bg-indigo-400 rounded-full" />}
            </button>
            ...
            <button onClick={() => setShowForm(true)}
              ...
              <FiPlus size={13} /> <span className="hidden xs:inline">New task</span>
            </button>
          </div>
        </header>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
          <div className="col-span-2 sm:col-span-1 bg-[#111120] border border-white/[0.06] rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-[8px] font-black text-blue-400 uppercase tracking-widest">Progress</p>
              <p className="text-2xl font-black text-white mt-0.5">{stats.progress}%</p>
              <p className="text-[9px] text-white/40 mt-1">{stats.completed}/{stats.total} done</p>
            </div>
          </div>
          <StatCard icon={<FiActivity size={15} />} label="Total"     value={stats.total}     accent="bg-sky-500/15 text-sky-400" />
          <StatCard icon={<FiZap size={15} />}      label="Active"    value={stats.pending}   accent="bg-amber-500/15 text-amber-400" />
          <StatCard icon={<FiAward size={15} />}    label="Completed" value={stats.completed} accent="bg-emerald-500/15 text-emerald-400" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          ...
          <div className="lg:col-span-2 bg-[#111120] border border-white/[0.06] rounded-2xl p-5">
            <div className="flex items-start justify-between mb-3">
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-white/25">This week's activity</p>
                <p className="text-[9px] text-white/15 mt-0.5">Based on task due dates (Sun – Sat)</p>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <span className="flex items-center gap-1.5 text-[9px] text-white/30">
                  <span className="w-2 h-2 rounded-full bg-indigo-500 inline-block" /> Added
                </span>
                <span className="flex items-center gap-1.5 text-[9px] text-white/30">
            ...
            <ResponsiveContainer width="100%" height={148}>
              <AreaChart data={weeklyData} margin={{ top: 4, right: 4, bottom: 0, left: -26 }}>
                <defs>
                ...
                <Area type="monotone" dataKey="added"     name="Added"     stroke={INDIGO}  strokeWidth={2} fill="url(#gAdded)" dot={false} activeDot={{ r: 4, fill: INDIGO }} />
                <Area type="monotone" dataKey="completed" name="Completed" stroke={EMERALD} strokeWidth={2} fill="url(#gDone)"  dot={false} activeDot={{ r: 4, fill: EMERALD }} />
              </AreaChart>
            </ResponsiveContainer>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mb-5">
          <div className="flex bg-[#111120] border border-white/[0.06] p-1 rounded-2xl gap-0.5 overflow-x-auto">
            {[
              ...
              { id: "timetable", label: "Timetable", icon: <FiZap size={11} /> },
              ...
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all ${
                  activeTab === t.id ? "bg-blue-600 text-white shadow-lg shadow-indigo-600/20" : "text-white/50 hover:text-white"
                }`}>
                {t.icon} {t.label}
              </button>
            ))}
            ...
            {["all", "pending", "done"].map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${
                  filter === f ? "bg-white/10 text-white" : "text-white/25 hover:text-white"
                }`}>
                {f}
              </button>
            ))}
          ...
          {activeTab === "roadmap" && (
            <motion.div key="roadmap" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              {filteredTasks.length === 0
                ...
                : <VisualRoadmap tasks={filteredTasks} onToggle={toggleTask} onDelete={deleteTask}
                    onEdit={setEditTask} page={roadmapPage} onPageChange={setRoadmapPage} />}
            </motion.div>
          )}

          {activeTab === "timetable" && (
            <motion.div key="timetable" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {timetablePeriods.map(period => (
                <div key={period.id} className="bg-[#111120] border border-white/[0.06] rounded-2xl p-5">
                  <div className="flex items-center gap-2 pb-4 mb-4 border-b border-white/[0.04]">
                    {period.icon}
                    <span className="text-xs font-black text-white uppercase tracking-wider">{period.label}</span>
                    <span className="ml-auto text-[9px] font-black text-white/40 bg-white/[0.03] px-2 py-0.5 rounded-full">{period.items.length}</span>
                  </div>
                  <div className="space-y-2.5">
                    {period.items.length === 0
                      ? <p className="text-[10px] text-white/50 text-center py-6">No tasks</p>
                      : period.items.map(t => (
                        <div key={t._id} className={`p-3.5 rounded-xl border group ${t.completed ? "border-indigo-500/10 bg-indigo-500/[0.02]" : "border-white/[0.04] hover:border-white/10"}`}>
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-[9px] font-mono text-indigo-400">{t.time || "--:--"}</span>
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                              <button onClick={() => setEditTask(t)} className="text-white/40 hover:text-indigo-400 transition"><FiEdit3 size={14} /></button>
                              {t.completed && <FiCheckCircle size={10} className="text-indigo-400" />}
                            </div>
                          </div>
                          <p className={`text-xs font-semibold leading-snug break-words ${t.completed ? "line-through text-white/40" : "text-white"}`}>{t.text}</p>
                        </div>
                      ))}
                  </div>
                </div>
            ...
            <motion.div key="grid" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              {filteredTasks.length === 0
                ...
                : <>
                        ...
                        <table className="w-full min-w-[520px] text-left">
                          <thead>
                            <tr className="border-b border-white/50">
                              {["#", "✓", "Objective", "Date", "Time", ""].map((h, i) => (
                                <th key={i} className="px-4 py-3.5 text-[8px] font-black uppercase tracking-widest text-white/70 whitespace-nowrap">{h}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {filteredTasks
                              .slice((gridPage - 1) * PAGE_SIZE, gridPage * PAGE_SIZE)
                              .map((t, i) => {
                                ...
                                return (
                                  <motion.tr key={t._id}
                                    ...
                                    <td className="px-4 py-3.5 text-[10px] font-mono text-white/50 w-8">{gIdx + 1}</td>
                                    <td className="px-4 py-3.5 w-10">
                                      <button onClick={() => toggleTask(t._id)}
                                        className={`w-6 h-6 rounded-lg border flex items-center justify-center transition-all ${
                                          t.completed ? "border-indigo-500 bg-green-900 text-indigo-400" : "border-white/10 text-white/50 hover:border-indigo-500/40"
                                        }`}>
                                        <FiCheck size={15} />
                                      </button>
                                    </td>
                                    <td className={`px-4 py-3.5 text-sm font-semibold max-w-[140px] sm:max-w-xs truncate ${t.completed ? "line-through text-white/30" : "text-white"}`}>
                                      {t.text}
                                    </td>
                                    <td className="px-4 py-3.5 text-[13px] font-mono text-white/50 whitespace-nowrap">{t.date || "—"}</td>
                                    <td className="px-4 py-3.5 text-[13px] font-mono text-white/50 whitespace-nowrap">{t.time || "—"}</td>
                    ...
                    <Pagination page={gridPage} total={filteredTasks.length} pageSize={PAGE_SIZE} onChange={setGridPage} />
                  </>
              }
            </motion.div>
          )}
